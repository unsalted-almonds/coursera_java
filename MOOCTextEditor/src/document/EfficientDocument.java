...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (String str : tokens){
			this.numSyllables = this.numSyllables + countSyllables(str);
			if(!this.isWord(str))
				this.numSentences++;
			else
				this.numWords++;
	    ...
	    //if last word is not ended in punctuation, still count it as a sentence 
		if(this.isWord(tokens.get(tokens.size()-1)))
			this.numSentences++;	
	}
