...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		// consult support video for using matrix multiplication for solving 2-hop 
		
		 List<Integer> res = new ArrayList<Integer>();
		 
		 if (v > this.getNumVertices() - 1)
			 return res;
		
		// the matrix contains 2 hop info for all vertices 
		int [][] resMatrix = new int[this.getNumVertices()][this.getNumVertices()];
		
		// this is not trivial 
		for (int i = 0; i < this.getNumVertices(); i++)
			for (int j = 0; j < this.getNumVertices(); j++)
				for (int k = 0; k < this.getNumVertices(); k++)
					resMatrix[i][j] += adjMatrix[i][k] * adjMatrix[k][j];
			
		// loop through v's 2 hop matrix
		for (int i = 0; i < this.getNumVertices() ; i++)
			for(int n = resMatrix[v][i]; n > 0; n--)
				//for passing the grader, don't include duplicates (this is the instructor's fault!)
				if(!res.contains(i))
					res.add(i);
