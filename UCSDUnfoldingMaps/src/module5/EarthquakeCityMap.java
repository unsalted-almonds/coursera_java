...
public class EarthquakeCityMap extends PApplet {
		...
		for (Marker m : markers) {
			if (m.isInside(this.map, mouseX, mouseY)) {
				if (m instanceof CommonMarker) {
					...
					// System.out.println("Marker is Hovered, set its attribute to true!");
				} else {
					// System.out.println("Some Marker is Hovered, but I cannot handle it");
				}
				break;
			}
		}
	...
	public void mouseClicked() {
		...
		selectEarthquakeMarkerIfClicked(quakeMarkers);
		selectCityMarkerIfClicked(cityMarkers);

	}

	// helper method for mouse click
	// getDistanceTo: Distance to location in kilometers (km).
	private void selectEarthquakeMarkerIfClicked(List<Marker> markers) {

		for (Marker quake : markers) {
			if (quake.isInside(this.map, mouseX, mouseY)) {
				for (Marker city : this.cityMarkers) {
					if (quake.getDistanceTo(city.getLocation()) <= ((EarthquakeMarker) quake)
							.threatCircle())
				...
				// hide everything
				for (Marker quake2 : markers){
					quake2.setHidden(true);
				}
				// show only itself
				((CommonMarker) quake).setHidden(false);
				this.lastClicked = (CommonMarker) quake;
	...
	// helper method for mouse click
	// getDistanceTo: Distance to location in kilometers (km).
	private void selectCityMarkerIfClicked(List<Marker> markers) {

		for (Marker city : markers) {

			if (city.isInside(this.map, mouseX, mouseY)) {
				...
				for (Marker city2 : markers){
					city2.setHidden(true);
				}
				
				((CommonMarker) city).setHidden(false);
				this.lastClicked = (CommonMarker) city;
	...
	private void addKey() {
		...
		triangle(tri_xbase, tri_ybase - CityMarker.TRI_SIZE, tri_xbase
				- CityMarker.TRI_SIZE, tri_ybase + CityMarker.TRI_SIZE,
				tri_xbase + CityMarker.TRI_SIZE, tri_ybase
						+ CityMarker.TRI_SIZE);
