...
public abstract class EarthquakeMarker extends CommonMarker {
	...
	public EarthquakeMarker(PointFeature feature) {
		...
		float magnitude = Float.parseFloat(properties.get("magnitude")
				.toString());
	...
	public void drawMarker(PGraphics pg, float x, float y) {
		...
		if ("Past Hour".equals(age) || "Past Day".equals(age)) {
			...
			pg.strokeWeight(2);
			int buffer = 2;
			pg.line(x - (radius + buffer), y - (radius + buffer), x + radius
					+ buffer, y + radius + buffer);
			pg.line(x - (radius + buffer), y + (radius + buffer), x + radius
					+ buffer, y - (radius + buffer));
	...
	public void showTitle(PGraphics pg, float x, float y) {
		...
		pg.fill(123, 24, 68);
		pg.text(this.getTitle() + ", " + this.getRadius() + ", "
				+ this.getDepth() + ", " + this.getMagnitude(), x, y);
