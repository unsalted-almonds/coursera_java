...
public class ParseFeed {
	...
	public static List<PointFeature> parseEarthquake(PApplet p, String fileName) {
		...
		for (int i = 0; i < itemXML.length; i++) {
			...
			if (titleStr != null) {
				...
				point.putProperty("magnitude",
						Float.parseFloat(titleStr.substring(2, 5)));
			}
	 ...
	 */
	public static HashMap<String, Float> loadLifeExpectancyFromCSV(PApplet p,
			String fileName) {
	...
	/*
	 * This method is to parse a file containing Out-of-pocket health
	 * expenditure information from
	 * 
	 * A map of maps (for getting numbers from multiple years) is used to store
	 ...
	 * @param fileName - file name or URL for data source
	 * 
	 * @return A HashMap of country->% of private expenditure on health by year
	 * (from 1995 - 2013)
	 */
	public static Map<String, Map<Integer, Double>> loadHealthExpenditureFromCSV(
			PApplet p, String fileName) {
		// HashMap key: country ID and data: % of private expenditure on health
		// by year
		Map<String, Map<Integer, Double>> expenditureMap = new HashMap<String, Map<Integer, Double>>();

		// get lines of csv file
		// fields in the target file contains double quotes, need to take them
		// out
		String[] rows = p.loadStrings(fileName);

		int dataStartIdx = 39;
		int dataEndIdx = 57;
		int countryCodeIdx = 1;
		...
		for (String row : rows) {
			...
			// a new map for each row/country
			Map<Integer, Double> yearlyData = new HashMap<Integer, Double>();

			// country code name is at columns[1]
			// data starts from year 1995 - columns[39] to year 2013 -
			// columns[57]
			for (int i = dataStartIdx; i <= dataEndIdx; i++) {
				...
				// check if value exists for year
				// if there's no data, it'll be an empty string ""
				if (!col.equals("")) {
					yearlyData.put(1995 + i - dataStartIdx,
							Double.parseDouble(col));
				}
			}

			// add into result map if any data recovered for the row
			if (!yearlyData.isEmpty())
				expenditureMap.put(columns[countryCodeIdx].replace("\"", ""), yearlyData);
